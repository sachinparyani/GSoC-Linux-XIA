...
static inline u32 rht_jhash2(const void *k, u32 length, u32 interval)
{
	const u8 *xid = k;
...
static inline int rht_obj_cmpfn(struct rhashtable_compare_arg *arg, const void *obj)
{
	...
	struct rht_fib_xid *rfxid = (struct rht_fib_xid *)obj;
	return (!are_xids_equal(rfxid_fxid(rfxid)->fx_xid, key));
}

static inline u32 rht_obj_hashfn(const void *data, u32 len, u32 seed)
{
	struct rht_fib_xid *rfxid = (struct rht_fib_xid *)data;
	BUILD_BUG_ON(XIA_XID_MAX != sizeof(u32) * 5);
...
static inline int __rhashtable_remove_fast_death(
...
static inline int rhashtable_remove_fast_death(
	...
	const struct rhashtable_params params)
{
	 ...
	 */
	while ((err = __rhashtable_remove_fast_death(ht, tbl, obj, params)) &&
...
{
	struct rhashtable_free_and_destroy_arg *rfd_arg = arg; 
	int *rm_count = rfd_arg->rm_count;
	struct rht_fib_xid *rfxid = ptr;
	struct fib_xid_table *xtbl = rfd_arg->xtbl;
	rhashtable_remove_fast_death(&xtbl_rxtbl(xtbl)->rht, &rfxid->node, (&xtbl_rxtbl(xtbl)->rht)->p);
...
static void rht_xtbl_death_work(struct work_struct *work)
{
	...
	rhashtable_free_and_destroy(&rxtbl->rht, free_fn, &arg);
...
restart:
	...
	rht_for_each_rcu(he, tbl, hash) {
		if (rht_params.obj_cmpfn ?
		    rht_params.obj_cmpfn(&arg, rht_obj(&rxtbl->rht, he)) :
	...
	struct fib_xid_table *xtbl, const u8 *xid) __acquires(xip_bucket_lock)
{
	...
	const struct bucket_table *tbl;
	struct rhash_head *he;
	unsigned int *phash = parg;
	spinlock_t *lock;
	
	tbl = (&rxtbl->rht)->tbl;
restart:
	*phash = rht_key_hashfn(&rxtbl->rht, tbl, xid, rht_params);
	lock = rht_bucket_lock(tbl, *phash);
	spin_lock_bh(lock);
	rht_for_each(he, tbl, *phash) {
		if (rht_params.obj_cmpfn ?
		    rht_params.obj_cmpfn(&arg, rht_obj(&rxtbl->rht, he)) :
		    rhashtable_compare(&arg, rht_obj(&rxtbl->rht, he)))
			continue;
				 ...
				 const void *arg)
{
	...
	while((rfxid = rhashtable_walk_next(&hti))) {
		...
		rc = rcu_callback(xtbl, rfxid_fxid(rfxid), arg);
		if(rc)
			goto out;
	}
...
static inline int __rhashtable_lookup_insert_key_locked(
				...
				struct fib_xid *fxid)
{
	...
	rc =  __rhashtable_lookup_insert_key_locked(parg, &rxtbl->rht, fxid->fx_xid, &rfxid->node, rht_params);
	if(IS_ERR(&rc))
		goto out;
			      ...
			      struct netlink_callback *cb)
{
	...
	for (i = cb->args[1]; i < tbl->size; i++, first_j = 0) {
		...
		j = 0;
		rht_for_each_entry_rcu(rfxid, head, tbl, i, node)
		{
			if (j < first_j)
				goto next;
